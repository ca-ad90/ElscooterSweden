...
export default async function deleteSanityCategory({
    event: { data },
    container,
    ...
    try {
        ...
        logger.info(`Deleted category ${data.id} from Sanity`);
    } catch (error) {
        logger.error(`Error deleting category ${data.id} from Sanity:`, error);
    }
}
