...
export default async function deleteSanityVariant({
    event: { data },
    container,
    ...
    try {
        ...
        logger.info(`Deleted variant ${data.id} from Sanity`);
    } catch (error) {
        logger.error(`Error deleting variant ${data.id} from Sanity:`, error);
    }
}

export const config: SubscriberConfig = {
    event: "product_variant.deleted",
};
